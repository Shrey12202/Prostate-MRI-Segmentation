...
def run_pipeline():
    ...
    if (cfg.preprocessed_dir / "manifest.csv").exists():
        print(f"⚠️ Skipping data cleaning — found existing manifest at {cfg.preprocessed_dir / 'manifest.csv'}")
    else:
        pairs = discover_pairs(cfg.raw_data_root)
        run_cleaning(pairs, cfg.preprocessed_dir)
        print(f"✅ Preprocessed data saved to {cfg.preprocessed_dir}")

    print("========== Step 2: Embedding Extraction ==========")
    cmd_embed = [
        "python", "-m", "src.medsam_embedder",
        "--checkpoint", str(cfg.medsam_ckpt),
        "--preprocessed_dir", str(cfg.preprocessed_dir),
        "--embeddings_dir", str(cfg.embeddings_dir)
    ]
