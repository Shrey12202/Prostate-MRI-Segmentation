...
class SliceDataset(Dataset):
    ...
    def _load_mask(self, p: str) -> torch.Tensor:
        mk = Image.open(p).convert("L")
        mk_t = (TF.pil_to_tensor(mk).float() / 255.0)  # (1,H,W) float in [0,1]
        mk_t = (mk_t > 0.5).float()                    # binarize
        return mk_t                                    # keep shape (1,H,W)
    ...
    def __getitem__(self, idx: int):
        ...
        mask = self._load_mask(row['mask'])  # (1,H,W)
        mask = mask.squeeze(0).unsqueeze(0)  # enforce shape exactly (1,H,W)
...
def make_loader(manifest_csv: Path, split_csv: Optional[Path], embeddings_dir: Path, batch_size: int, shuffle: bool, augment: bool, num_workers: int):
    ...
    # return DataLoader(ds, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, pin_memory=True, persistent_workers=True)  # added persistent_workers=True,
